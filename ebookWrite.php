 ...
 ***********/
  ...
  private function buildFiles(){
    echo "execution trace : appel à builFiles<br/>"; // DEBUG
    //Make all the files needed for the epub file.
    $metaFolder = $this->ebookData->tempDir."META-INF";
    if(!is_dir($metaFolder)) {
      mkdir($metaFolder, 0755); // correction en local : ajout d'un 0
    }
    if(!isset($this->ebookData->contentFolder)){
      $oebpsFolder = $this->ebookData->tempDir."OEBPS";
      if(!is_dir($oebpsFolder)) { // il fait chier à pas mettre d'accolades (et est-ce que le test d'existence est inclus ?)
        mkdir($oebpsFolder, 0755); // correction en local : ajout d'un 0
        echo "execution trace : dans buildFile, création d'un OEPBS folder<br/>"; // DEBUG
      }
      $this->ebookData->contentFolder = $this->ebookData->tempDir."OEBPS/";
      echo "execution trace (dans buildFile : this->ebookData->contentFolder = ".$this->ebookData->contentFolder."<br/>";
   ...
   */
   ...
   private function packageEpub($name, $dest){
     echo "execution trace : appel à packageEpub<br/>"; // DEBUG
    ...
    echo "execution trace (packageEpub) : changement de répertoire pour ".$this->ebookData->tempDir."<br/>"; // DEBUG
    ...
    // add files
    foreach($list as $f) {
      echo "execution trace (packageEpub) : ajout du fichier ".$f."<br/>"; // DEBUG
        $f = str_replace($this->ebookData->tempDir, "", $f); // on enlève le chemin dans le nom de fichier, et s'il ne correspond pas ?
        $zipfile->addFile($f) or trigger_error("Could not add file: ".$f." to the epub.", E_USER_ERROR); // astuce qui semble dépendante de l'implémentation plutôt que de l'algorithmique
    }
    $zipfile->close();
...
<container version=\"1.0\" xmlns=\"urn:oasis:names:tc:opendocument:xmlns:container\">
  ...
  private function addManifestItem($id, $href, $media, $fall = null){
    echo "executionTrace : appel à addManifestItem avec id = ".$id."<br/>";
    $man = new manifest();
    $man->id = $id;
    $man->href = $href;
    $man->type = $media;
   ...
   */
   ...
   private function buildSingleTag($main, $opt1 = null, $opt2 = null, $opt3 = null, $opt4 = null){
    ...
    $string = "<".$main; // au lieu de $string = $string."<".$main; (utilisation de la variable avant de l'initialiser, et php permet ça !)
    if(isset($opt1)){
   ...
   */
   ...
   private function existInManifest($item){
    $pass = false;
    /*
    foreach($this->ebookData->manifestData as $manId){ // on ne se soucie même pas de si le manifeste est un tableau ou pas !!!
    ...
    * */
    ...
    if(isset($this->ebookData->manifestData)){
      if(is_array($this->ebookData->manifestData)){
        ...
        if ($item == (string)$this->ebookData->manifestData->id) {
          $pass = true;
        }
      }
    }
 ...
 **********/
  ...
  public function buildEPUB($name, $dest){
    echo "execution trace : buildEPUB<br/>";
 ...
 *********************/
  ...
  public function addContentFile($fileLoc, $name, $mime, $subDir = null, $fallBackId = null){
    echo "execution trace : appel de addContentFile avec un id = ".$name."<br/>";
    //Check that we have a file
    if((!empty($fileLoc))) {
      $filename = basename($fileLoc);

      if(!isset($this->ebookData->contentFolder)){
        mkdir($this->ebookData->tempDir."OEBPS",  0755);
        $this->ebookData->contentFolder = $this->ebookData->tempDir."OEBPS/";
      }

      if(!file_exists($this->ebookData->contentFolder.$subDir))
        $this->mkdir_recursive($this->ebookData->contentFolder.$subDir, 0755);

      $newname = $this->ebookData->contentFolder.$subDir.$filename;
       echo "execution trace (addContentFile) : on met le nom = ".$newname."<br/>";
